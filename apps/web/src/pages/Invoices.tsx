...
function ReceiptPrintView(props: {
  ...
  return (
    <PrintOverlay
      ...
      screenNote={
        "此證明聯的左側 QR 加密驗證區（24 碼）目前以佔位字元填入：正式驗真值需以財政部大平台核發的 " +
        "QRCode 專用金鑰產生，本系統尚未申請。號碼、日期、金額、統編等明碼欄位皆可正常掃描核對。"
      }
    >
      <div className="r-store">{company.name}</div>
      <div className="r-title">電子發票證明聯</div>
      ...
      <div className="r-line">{inv.invoiceDate}　隨機碼 {inv.randomNumber}　總計 {fmt(inv.totalAmount)}</div>
      <div className="r-line">
        賣方 {company.taxId}
        {inv.buyerTaxId ? `　買方 ${inv.buyerTaxId}` : ""}
      </div>
...
export function Invoices() {
  ...
  const openReceipt = async (inv: InvoiceRow) => {
    try {
      if (!company.data) throw new Error("公司基本檔未設定，證明聯缺賣方統編——請先到「設定」頁填寫");
      if (inv.saleId === null) throw new Error("處分發票沒有銷貨單明細，無法產生證明聯——請以 XML 為準");
  ...
  const carrierText = (inv: InvoiceRow) => {
    if (inv.donateMark === "1") return `捐贈 ${inv.npoban ?? ""}`.trim();
    if (inv.carrierType) return `${inv.carrierType === "3J0002" ? "手機條碼" : inv.carrierType === "CQ0001" ? "自然人憑證" : inv.carrierType} ${inv.carrierId ?? ""}`.trim();
    return "—";
  };
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <table>
          <thead>
            <tr><th>發票號碼</th><th>日期</th><th>類型</th><th title="銷貨＝對銷貨單開立；處分＝對固定資產處分開立（0034）">來源</th><th>課稅別</th><th>買受人</th><th title="B2C 的載具或捐贈碼（開立當下的快照）；「—」＝無載具（紙本證明聯）或 0029 前的舊發票">載具／捐贈</th><th className="num">總額</th><th>狀態</th><th></th></tr>
          </thead>
          <tbody>
            {invoices.data?.map((inv) => (
              <tr key={inv.id}>
                ...
                <td>{inv.mode}</td>
                <td>{inv.saleId ? `銷貨 #${inv.saleId}` : `處分資產 #${inv.assetId}`}</td>
                {/* 課稅別（0028）：開立當下的快照。零稅率＝稅額 0、金額在 ZeroTaxSalesAmount */}
                <td>{inv.taxType === "2" ? <span className="badge">零稅率</span> : "應稅"}</td>
                <td>{inv.buyerName}</td>
                <td>{carrierText(inv)}</td>
                <td className="num">{fmt(inv.totalAmount)}</td>
                <td><span className={`badge ${inv.status}`}>{inv.status === "issued" ? "已開立" : "已作廢"}</span></td>
                <td>
                  ...
                  {inv.status === "issued" && inv.saleId !== null && (
                    <>
                      <button className="small" onClick={() => void openReceipt(inv)}>證明聯</button>{" "}
                    </>
                  )}
                  {inv.status === "issued" && cancelingId !== inv.id && (
                    <button className="small" onClick={() => { setCancelingId(inv.id); setReason(""); }}>作廢</button>
                  )}
                  {cancelingId === inv.id && (
                    <>
                      <input
                      ...
                      <button className="small" onClick={() => cancel(inv, true)}>
                        {inv.saleId ? "作廢並沖銷銷貨單" : "作廢並沖回處分"}
                      </button>{" "}
                      <button className="small" onClick={() => cancel(inv, false)}>僅作廢（將重開發票）</button>{" "}
                      <button className="small" onClick={() => setCancelingId(null)}>取消</button>
                      ...
                      {inv.saleId ? (
                        <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 4 }}>
                          ...
                          <br />
                          <strong>貨已經出去、客戶要退貨或要折價</strong> → 不要用作廢，請到「銷貨」頁對那張銷貨單按「退貨／折讓」，
                          以退貨當期入帳。<strong>發票日期所屬月份已關帳時作廢會被擋下</strong>
                          （作廢改的是那一期、可能已申報的銷項數字），此時唯一的路就是開退貨／折讓單。
                        </div>
                      ) : (
                        <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 4 }}>
                          處分發票：「沖回處分」＝反向傳票沖回處分損益／累計折舊／銷項稅額，資產回到使用中（處分本身是誤操作時用）。
                          僅作廢＝發票開錯、處分沒錯——帳不動，請至「資產」頁對這筆處分重開發票，否則 2288 銷項稅額不會進 401。
                        </div>
                      )}
                    </>
                  )}
      ...
      {xmlView && (
        <div className="card">
          <h3>{xmlView.title} <button className="small" onClick={() => setXmlView(null)}>關閉</button></h3>
