 ...
 */
import { bpToPercentText, percentToBp, withheldByRate } from "@tw-erp/core";
  ...
  return (
    <PrintOverlay onClose={props.onClose}>
      <div className="print-head">
        <div className="doc-title" style={{ marginTop: 0 }}>扣繳憑單套印參考</div>
        <div className="co-meta">{props.year} 年度（依給付日歸年）　※ 非財政部格式，僅供留存與交付受領人核對</div>
      </div>
      <table>
        <tbody>
          <tr>
            <th style={{ width: "28%" }}>格式代號（所得類別）</th>
            <td>{blank}（請依申報軟體之代號自行填寫）</td>
          </tr>
          <tr>
            <th>扣繳單位</th>
            <td>
              {company
                ? `${company.name}（統一編號 ${company.taxId}）${company.address ?? ""}${company.personInCharge ? `　負責人 ${company.personInCharge}` : ""}`
                : "（公司基本檔未設定——請至「設定」頁填寫）"}
            </td>
          </tr>
          <tr><th>所得人（受領人）</th><td>{row.partnerName}</td></tr>
          <tr>
            <th>身分證統一編號</th>
            <td>{props.idNo ?? "（未填——請先到「客戶與商品」頁補登，否則憑單填不出來）"}</td>
          </tr>
          <tr><th>給付內容（本系統類別）</th><td>{row.categoryLabel}（{row.count} 筆）</td></tr>
          <tr><th className="num">給付總額</th><td className="num">{fmt(row.grossAmount)} 元</td></tr>
          <tr><th className="num">扣繳稅額</th><td className="num">{fmt(row.taxWithheld)} 元</td></tr>
          <tr><th className="num">給付淨額</th><td className="num">{fmt(row.grossAmount - row.taxWithheld)} 元</td></tr>
          <tr>
            <th className="num">代扣補充保費（參考）</th>
            <td className="num">{fmt(row.supplementWithheld)} 元（不屬扣繳憑單欄位，列出供對帳）</td>
          </tr>
        </tbody>
      </table>
      {(row.unsetTaxRateCount > 0 || row.unsetSupplementRateCount > 0) && (
        <div className="foot-note" style={{ fontWeight: 600 }}>
          ⚠ 本列有 {row.unsetTaxRateCount + row.unsetSupplementRateCount} 筆在費率尚未設定時建立，代扣以 0 計
          ——不代表不用扣。請先回「扣繳」頁核對更正後再套印。
        </div>
      )}
      <div className="sign-row">
        <div className="sign-box">扣繳義務人簽章</div>
        <div className="sign-box">所得人簽收</div>
      </div>
      <div className="foot-note">
        金額為整數新台幣元。正式憑單請以財政部申報軟體產製；申報與繳納期限本系統不提示，請依主管機關公告辦理。
      </div>
    </PrintOverlay>
  );
}
...
function rateText(bp: number | null): string {
  return bp === null ? "尚未設定" : `${bpToPercentText(bp)}%`;
}
...
export function Withholding() {
  ...
  const voidPayment = async (p: WithholdingPaymentRow) => {
    const reason = window.prompt(
      `作廢扣繳支出單 #${p.id}（${p.partnerName}，給付總額 ${fmt(p.grossAmount)} 元）：請輸入作廢理由。\n` +
        "反向傳票會沖平費用／代扣款／現金，這張單也不再計入年度彙總；金額打錯請作廢後重開一張。",
    );
  ...
  const submitPayment = async () => {
    try {
      if (!partnerId) throw new Error("請選擇受領人（只有勾了「個人」的交易對象會出現在這裡）");
      if (!categoryId) throw new Error("請選擇扣繳類別");
      if (grossAmount <= 0) throw new Error("給付總額須為正整數（填扣繳前的總額，不是實際匯出去的錢）");
      if (!cashAccounts.length) throw new Error("沒有可用的現金科目，請到「會計科目」頁把銀行帳戶勾為現金科目");
  ...
  const reveal = async (id: number) => {
    try {
      ...
      setRevealed((r) => ({ ...r, [id]: row.idNo ?? "（未填）" }));
      setError(null);
    } catch (e) {
      setError((e as Error).message);
    }
    ...
    <div>
      ...
      <div className="card">
        ...
        <p style={{ margin: "0 0 6px" }}>
          <strong>會做</strong>：把「付給個人的錢」記成一張單——認列費用、代扣的稅款與補充保費、實際付出去的金額，
          並自動產生會計分錄（借費用／貸 2211 代扣所得稅、2212 代扣勞健保費、貸 銀行或現金）。
          下方的年度彙總依「受領人 × 類別」加總，就是填各類所得憑單的取數來源。
        </p>
        <p style={{ margin: "0 0 6px" }}>
          <strong>不會做</strong>：系統<strong>不內建任何費率</strong>。扣繳率、補充保費費率請你自己查證後填進下方的類別設定，
          並在「依據來源」欄寫下你查到的出處與查詢日期——這些數字每年可能變動，由系統代你斷言只會讓錯誤被當成權威。
          系統也<strong>不產生憑單媒體檔</strong>（固定長度欄位的格式規格尚未取得，見 <code>docs/specs/withholding.md</code>），
          請以財政部的申報軟體填報；本頁的彙總數字是拿來抄進那套軟體的。
          系統同樣<strong>不提示任何繳納或申報期限</strong>——期限本專案未查證，請依國稅局公告辦理。
        </p>
        <p style={{ margin: 0, fontSize: 13, color: "var(--text-2)" }}>
          費率填了之後，建單時會自動試算成預設值，但<strong>永遠可以改</strong>：小額給付的免扣門檻、身分別的例外
          這些規則系統沒有模型，你手上的繳款單才是準的。
        </p>
      </div>

      {/* --- 類別設定 --- */}
      <div className="card">
        <h3>扣繳類別與費率（費率由你填寫，並請註明依據來源）</h3>
        <table>
          <thead>
            <tr>
              <th>類別</th><th>費用科目</th><th className="num">扣繳率</th><th className="num">補充保費率</th>
              <th>依據來源</th><th>狀態</th><th></th>
            </tr>
          </thead>
          <tbody>
        ...
        >
          <label className="field">新增類別（白話名稱）<input name="label" required placeholder="例如：付給個人的翻譯費" /></label>
          ...
          <button className="primary">新增類別</button>
        </form>
        <p style={{ fontSize: 13, color: "var(--text-2)" }}>
          新增的類別費率一律是空的（＝尚未設定），建單時會以 0 計並提醒你。
          填 0 和留空不一樣：<strong>0 代表「我查過，這類不用扣」</strong>，留空代表「還沒查」。
        </p>
      </div>
      ...
      <div className="card">
        ...
        {individuals.length === 0 ? (
          <EmptyState
            icon="🧾"
            title="還沒有「個人」的交易對象"
            desc="這張單只用於付款給自然人（個人房東、個人接案者）。請先到「客戶與商品」頁新增交易對象並勾選「個人」——已經建過的對象也可以在那裡改成個人。"
            actionLabel="去建立個人交易對象"
            actionPage="masters"
          />
        ) : (
          <>
            <form className="inline" onSubmit={(e) => e.preventDefault()}>
              <label className="field">
                ...
                <select value={partnerId} onChange={(e) => setPartnerId(Number(e.target.value))}>
                  ...
                  {individuals.map((p) => (
                    <option key={p.id} value={p.id}>{p.name}{p.hasIdNo ? "" : "（未填身分證號）"}</option>
                  ))}
                </select>
              </label>
              <label className="field">
                ...
                >
                  ...
                  {activeCategories.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.label}（扣繳率 {rateText(c.taxRateBp)}／補充保費率 {rateText(c.supplementRateBp)}）
                    </option>
                  ))}
                </select>
              </label>
              <label className="field">給付日<input type="date" value={payDate} onChange={(e) => setPayDate(e.target.value)} /></label>
              <label className="field">
                給付總額（扣繳前）
                <input
                  type="number"
              ...
              <label className="field">摘要<input value={memo} onChange={(e) => setMemo(e.target.value)} /></label>
              <button className="primary" onClick={() => void submitPayment()}>建立</button>
            </form>
            <div className="stat-row" style={{ marginTop: 12 }}>
              <div className="stat"><div className="label">認列費用（給付總額）</div><div className="value">{fmt(grossAmount)}</div></div>
              <div className="stat"><div className="label">代扣合計</div><div className="value">{fmt(taxValue + supplementValue)}</div></div>
              <div className="stat">
                <div className="label">實付給對方</div>
            ...
            {category && category.taxRateBp === null && (
              <div className="notice" style={{ marginTop: 12 }}>
                「{category.label}」<strong>尚未設定扣繳率</strong>，代扣所得稅預設為 0
                ——這不代表不用扣，只代表系統還不知道要扣多少。
                請在上方的類別設定填入你查到的費率並註明依據來源；或直接在這張單填入正確的代扣金額。
              </div>
            )}
            {category && category.supplementRateBp === null && (
              <div className="notice" style={{ marginTop: 12 }}>
                「{category.label}」<strong>尚未設定補充保費率</strong>，代扣補充保費預設為 0
                ——同樣不代表不用扣。這一類支出要不要扣補充保費請自行查證，
                查到後填入類別設定，或直接在這張單填入金額。
              </div>
            )}
            {category && (category.taxRateBp !== null || category.supplementRateBp !== null) && !category.sourceNote && (
              <div className="notice" style={{ marginTop: 12 }}>
                「{category.label}」的費率有填但沒有依據來源。
                建議在類別設定的「依據來源」欄寫下出處與查詢日期——明年這個數字若變了，你會需要知道當初是照哪裡填的。
              </div>
            )}
            {netPreview < 0 && (
              <div className="error">代扣合計超過給付總額，實付金額會變成負數。給付總額請填「扣繳前」的金額。</div>
            )}
          </>
        )}
      </div>
      ...
      <div className="card">
        ...
        {payments.data?.length === 0 ? (
          <EmptyState
            icon="📄"
            title="還沒有扣繳支出單"
            desc="每次付租金或委外費用給個人時開一張，系統會同時記好費用、代扣稅款與實付金額；年底的憑單申報就靠這些單彙總。"
          />
        ) : (
          <table>
            <thead>
              <tr>
                <th>給付日</th><th>受領人</th><th>類別</th>
                <th className="num">給付總額</th><th className="num">代扣所得稅</th>
                <th className="num">代扣補充保費</th><th className="num">實付</th>
                <th>付款科目</th><th>摘要</th><th>傳票</th><th></th>
              </tr>
            </thead>
            <tbody>
              {payments.data?.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>
                    {p.voidedAt ? (
                      <span className="badge canceled" title={`作廢理由：${p.voidReason ?? ""}`}>已作廢</span>
                    ) : (
                      <button className="small" onClick={() => void voidPayment(p)}>作廢</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="card">
        ...
        {summary && (summary.total.unsetTaxRateCount > 0 || summary.total.unsetSupplementRateCount > 0) && (
          <p style={{ margin: "0 0 8px", fontSize: 13, color: "var(--red)" }}>
            ⚠ 本年度有
            {summary.total.unsetTaxRateCount > 0 && <strong> {summary.total.unsetTaxRateCount} 筆</strong>}
            {summary.total.unsetTaxRateCount > 0 && "在尚未設定扣繳率時建立"}
            {summary.total.unsetTaxRateCount > 0 && summary.total.unsetSupplementRateCount > 0 && "、"}
            {summary.total.unsetSupplementRateCount > 0 && <strong> {summary.total.unsetSupplementRateCount} 筆</strong>}
            {summary.total.unsetSupplementRateCount > 0 && "在尚未設定補充保費率時建立"}
            ，那些代扣金額是<strong>以 0 計</strong>——這不代表不用扣，只代表系統當時不知道要扣多少。
            下表以紅字標出。補設費率<strong>不會回頭重算已建立的單據</strong>，
            需要更正的請自行判斷處理方式。
          </p>
        )}
        ...
        {summary && (
          <>
            <table style={{ marginTop: 12 }}>
              <thead>
                <tr>
                  <th>受領人</th><th>類別</th><th className="num">筆數</th>
                  <th className="num">給付總額</th><th className="num">代扣所得稅</th>
                  <th className="num">代扣補充保費</th><th className="num">實付</th><th>身分證號</th><th></th>
                </tr>
              </thead>
              <tbody>
                {summary.rows.map((r) => (
                  <tr key={`${r.partnerId}:${r.categoryId}`}>
                    ...
                    <td className="num" style={r.unsetTaxRateCount > 0 ? { color: "var(--red)" } : undefined}>
                      ...
                      {r.unsetTaxRateCount > 0 && (
                        <span title={`其中 ${r.unsetTaxRateCount} 筆建立時尚未設定扣繳率，代扣以 0 計`}>
                          {" "}⚠{r.unsetTaxRateCount}
                        </span>
                      )}
                    </td>
                    <td className="num" style={r.unsetSupplementRateCount > 0 ? { color: "var(--red)" } : undefined}>
                      ...
                      {r.unsetSupplementRateCount > 0 && (
                        <span title={`其中 ${r.unsetSupplementRateCount} 筆建立時尚未設定補充保費率，代扣以 0 計`}>
                    ...
                    <td>
                      {revealed[r.partnerId] ? (
                        ...
                        <button className="small" onClick={() => void reveal(r.partnerId)}>顯示</button>
                      ) : (
                        <span style={{ color: "var(--red)" }}>未填</span>
                      )}
                    </td>
                    {/* 套印一張憑單參考單（受領人×類別＝一張）：正式憑單仍以申報軟體產製 */}
                    <td><button className="small" onClick={() => void openSlip(r)}>套印</button></td>
                  </tr>
                ))}
                {summary.rows.length === 0 && (
                  <tr><td colSpan={9}>{summary.year} 年沒有扣繳支出紀錄。</td></tr>
                )}
              </tbody>
              {summary.rows.length > 0 && (
                <tfoot>
            ...
            <p style={{ margin: "12px 0 6px" }}>
              這是申報憑單的取數來源。本系統<strong>不產生</strong>憑單媒體檔
              （固定長度欄位的格式規格尚未取得，見 <code>docs/specs/withholding.md</code>），
              請以財政部的申報軟體填報，並自行核對每一位受領人的金額與身分資料。
              身分證號沒填的受領人請先補填，否則憑單填不出來（在「客戶與商品」頁補）。
            </p>
            ...
            <p style={{ fontSize: 13, color: "var(--text-2)", margin: "6px 0 0" }}>
              上面兩個餘額是<strong>已經從對方身上扣下來、但還沒繳出去</strong>的稅款與保費（不分年度，是「現在還欠多少」）。
              繳款之後請到「傳票」頁開一張手工傳票沖掉它（借 2211／2212、貸 銀行存款），餘額就會回到 0。
              <strong>系統不提示繳納期限</strong>——期限請依國稅局／健保署的公告辦理。
            </p>
          </>
        )}
      </div>
...
function CategoryRow(props: {
  ...
  const save = async () => {
    ...
    if (taxBp === undefined || supplementBp === undefined) {
      props.onError("費率請填 0 到 100 之間的百分比，可帶小數（例如 3.5）；留空代表尚未設定");
      return;
    }
    try {
  ...
  return (
    <tr>
      <td>{c.label}</td>
      <td>
        {c.expenseAccountCode} {accountName?.name ?? "（科目不存在）"}
        {accountName && !accountName.active && <span style={{ color: "var(--red)" }}>（已停用，開單會被擋）</span>}
      </td>
      {editing ? (
        <>
          <td className="num">
          ...
          <td>
            <input
              ...
              placeholder="例如：依據 XXX 頁面，查詢日 2026-07-30"
            />
          </td>
          <td>{c.active ? "啟用" : "停用"}</td>
          <td>
            <button className="small" onClick={() => void save()}>儲存</button>{" "}
        ...
        <>
          <td className="num" style={c.taxRateBp === null ? { color: "var(--amber)" } : undefined}>
            {rateText(c.taxRateBp)}
          </td>
          <td className="num" style={c.supplementRateBp === null ? { color: "var(--amber)" } : undefined}>
            {rateText(c.supplementRateBp)}
          </td>
          <td>
            {c.sourceNote ?? (
              <span style={{ color: "var(--amber)" }}>
                {c.taxRateBp === null && c.supplementRateBp === null ? "—" : "未註明依據來源"}
              </span>
            )}
          </td>
          <td><span className={`badge ${c.active ? "issued" : "canceled"}`}>{c.active ? "啟用" : "停用"}</span></td>
          <td>
            <button className="small" onClick={() => setEditing(true)}>填費率／來源</button>{" "}
            <button className="small" onClick={() => void toggleActive()}>{c.active ? "停用" : "啟用"}</button>
          </td>
        </>
      )}
    </tr>
