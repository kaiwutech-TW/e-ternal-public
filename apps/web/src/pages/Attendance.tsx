...
export function Attendance() {
  ...
  const submitRequest = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    void act(async () => {
      ...
      form.reset();
      setOk(res.autoApproved ? "已送出並自動核准（你沒有可簽核的主管）" : "申請已送出，等待簽核");
    });
  };
  ...
  if (data?.notLinked) {
    return (
      <div className="card">
        <p>你的帳號還沒連結員工主檔，無法打卡。請管理者到「設定 → 使用者管理」把你的帳號連上員工。</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="card" style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: "0.78125rem", color: "var(--text-3)", marginTop: 12 }}>
          打卡記錄的是事實：打錯方向不用慌，補一筆正確的即可（紀錄不可修改，異常用下面的補卡申請更正）。
        </p>
      </div>

      {toSign.length > 0 && (
        <div className="card">
          <h3>待我簽核（{toSign.length}）</h3>
          <table>
            <thead><tr><th>申請人</th><th>類型</th><th>內容</th><th>事由</th><th>意見</th><th></th></tr></thead>
            <tbody>
              {toSign.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    <button
                      ...
                      disabled={busy}
                      onClick={() => void act(() => api.post(`/hr-requests/${r.id}/approve`, { comment: comments[r.id] ?? "" }), "已核准")}
                    >
                      核准
                    </button>{" "}
                    <button
                      className="small"
                      disabled={busy}
                      onClick={() => void act(() => api.post(`/hr-requests/${r.id}/reject`, { comment: comments[r.id] ?? "" }), "已駁回")}
                    >
                      駁回
                    </button>
                  </td>
            ...
            <>
              <label className="field">
                ...
                <select name="leaveTypeId" required defaultValue="">
                  ...
                  {leaveTypes.filter((t) => t.active).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                </select>
              </label>
              <label className="field">從<input name="startAt" type="datetime-local" required /></label>
              <label className="field">到<input name="endAt" type="datetime-local" required /></label>
              <label className="field">時數（分鐘）<input name="minutes" type="number" min={1} required placeholder="扣午休後的實際分鐘" /></label>
            </>
          )}
          {kind === "overtime" && (
            <>
              <label className="field">加班日期<input name="workDate" type="date" required /></label>
              <label className="field">
                日型
                <select name="dayType" required defaultValue="">
                  <option value="" disabled>— 選日型 —</option>
                  {Object.entries(DAY_TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                </select>
              </label>
              <label className="field">時數（分鐘）<input name="minutes" type="number" min={1} required /></label>
            </>
          )}
          {kind === "punch_correction" && (
            <>
              <label className="field">出勤日<input name="workDate" type="date" required /></label>
              <label className="field">
                方向
              ...
              <label className="field">時刻<input name="claimedTime" type="time" required /></label>
            </>
          )}
          <label className="field" style={{ minWidth: 200 }}>事由<input name="reason" placeholder="（選填）" /></label>
          <button className="primary" disabled={busy}>送出</button>
        </form>
        {kind === "overtime" && (
          <p style={{ fontSize: "0.78125rem", color: "var(--text-3)" }}>
            這天是平日、休息日、例假日還是國定假日由你自己選——加班費倍率按日型計算，選錯會算錯錢。
          </p>
        )}
      </div>
      ...
      {balances.length > 0 && (
        <div className="card">
          <h3>我的假別額度（{new Date().getFullYear()} 年）</h3>
          <table>
            <thead><tr><th>假別</th><th>給假</th><th>已用</th><th>簽核中</th><th>剩餘</th></tr></thead>
            <tbody>
              {balances.map((b) => (
                <tr key={b.id}>
      ...
      {mine.length > 0 && (
        <div className="card">
          <h3>我的申請</h3>
          <table>
            <thead><tr><th>類型</th><th>內容</th><th>狀態</th><th>簽核進度</th><th></th></tr></thead>
            <tbody>
              {mine.map((r) => (
                <tr key={r.id}>
                  <td>{KIND_LABELS[r.kind]}</td>
                  ...
                  <td>
                    {r.status === "pending" && (
                      <button className="small" disabled={busy} onClick={() => void act(() => api.post(`/hr-requests/${r.id}/cancel`, {}), "已取消")}>
                        取消
                      </button>
                    )}
                  </td>
      ...
      <div className="card">
        ...
        {data?.punches.length ? (
          <table>
            <thead><tr><th>時間</th><th>方向</th><th>方式</th></tr></thead>
            <tbody>
              {data.punches.map((p) => (
                <tr key={p.id}>
                  <td>{hhmm(p.punchedAt)}</td>
                  <td><span className={`badge ${p.direction === "in" ? "issued" : "canceled"}`}>{p.direction === "in" ? "上班" : "下班"}</span></td>
                  <td>{p.method === "web" ? "網頁" : p.method === "correction" ? "補卡" : p.method}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ) : (
          <p style={{ color: "var(--text-2)" }}>今天還沒有打卡。</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        {data?.schedule.length ? (
          <table>
            <thead><tr><th>日期</th><th>班別</th><th>時間</th><th>備註</th></tr></thead>
            <tbody>
              {data.schedule.map((s) => (
                <tr key={s.workDate} style={s.workDate === today ? { background: "var(--accent-tint)" } : undefined}>
                  <td>{s.workDate}{s.workDate === today && "（今天）"}</td>
                  <td>
          ...
          <p style={{ color: "var(--text-2)" }}>這兩週沒有排班（固定班的同事看這裡是空的屬正常，排班制請等主管排班）。</p>
        )}
      </div>
    </div>
  );
