...
export function ReturnForm(props: {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!lines.length) throw new Error(kind === "return" ? "請填要退的數量" : "請填折讓金額");
      if (cashOut && !cashAccountId) throw new Error("請選擇退款用的現金／銀行科目");
  ...
  if (!info) return <div style={{ fontSize: 13, color: "var(--text-2)" }}>讀取原單明細中…</div>;
    ...
    <div style={{ padding: "10px 0", borderTop: "1px solid rgba(0,0,0,0.08)" }}>
      <form className="inline" onSubmit={(e) => e.preventDefault()}>
        <label className="field">
          ...
          <select value={kind} onChange={(e) => setKind(e.target.value as ReturnKind)}>
            <option value="return">{label.ret}</option>
            <option value="allowance">{label.allow}</option>
          </select>
        </label>
        <label className="field">
          {kind === "return" ? "退貨日期" : "折讓日期"}
      ...
      */}
      <p style={{ fontSize: 12.5, color: "var(--amber)", background: "var(--amber-tint)", border: "1px solid var(--amber)", borderRadius: 6, padding: "6px 10px", margin: "8px 0 0" }}>
        本系統<strong>不會產生退回折讓證明單</strong>（電子折讓證明單 XML 尚未實作）。
        證明單需另行以財政部電子申報繳稅軟體或加值中心平台開立；上面兩個欄位是用來登錄那張單的號碼與日期，
        填了不代表已開立。營業稅申報時的退回減項也還要人工填（見「營業稅申報」頁的提示）。
      </p>
      ...
      <table style={{ marginTop: 10 }}>
        <thead>
          <tr>
            ...
            <th className="num">原金額</th>
            <th className="num">已退</th>
            <th className="num">{kind === "return" ? "這次退幾個" : "折讓金額（未稅）"}</th>
          </tr>
        </thead>
        <tbody>
          {info.lines.map((l) => (
            <tr key={l.id}>
              ...
              <td className="num">
                {l.returnedQty > 0 || l.returnedAmount > 0 || l.allowanceAmount > 0
                  ? `退 ${l.returnedQty} 個／${fmt(l.returnedAmount)} 元` +
                    (l.allowanceAmount > 0 ? `、折讓 ${fmt(l.allowanceAmount)} 元` : "")
                  : "—"}
              </td>
              <td className="num">
                {kind === "return" ? (
                ...
                <div style={{ fontSize: 12, color: "var(--text-2)" }}>
                  {kind === "return"
                    ? `最多 ${Math.max(0, l.remainingQty)}${
                        side === "purchase" && l.onHandQty !== undefined ? `（在庫 ${l.onHandQty}）` : ""
                      }`
                    : `最多 ${fmt(l.remainingAllowance)} 元`}
                </div>
              </td>
            </tr>
          ))}
      ...
      <form className="inline" style={{ marginTop: 10 }} onSubmit={(e) => e.preventDefault()}>
        ...
        {canUseCash ? (
          ...
          <span style={{ fontSize: 12.5, color: "var(--text-2)" }}>
            {label.cash}需要「收付款」頁的權限（會直接動到現金／銀行科目），您的角色沒有；
            這張單會自動沖抵貨款，剩下的掛{side === "sale" ? "其他應付款" : "其他應收款"}，由財務退款。
          </span>
        )}
        {cashOut && (
          <label className="field">
      ...
      <p style={{ fontSize: 12.5, color: "var(--text-2)", margin: "8px 0 0" }}>
        {side === "sale"
          ? "系統會先沖掉這位客戶還欠的貨款；已經付清的部分會掛在「其他應付款」（＝該退還給客戶的錢），之後可用付款單退還或抵下次交易。"
          : "系統會先沖掉還沒付給廠商的貨款；已經付清的部分會掛在「其他應收款」（＝廠商該退還給我們的錢）。"}
        {kind === "return"
          ? side === "sale"
            ? " 退回的商品會回到庫存，成本按原銷貨成本回沖（等於那次出貨沒發生過）。"
            : " 退出的商品按目前帳上的平均成本從庫存扣除；廠商退的錢與該成本的差額會調整銷貨成本" +
              "（退的錢比帳上成本多就減少成本、少就增加成本）。"
          : side === "sale"
            ? " 折讓不動庫存數量。"
            : " 折讓不動庫存數量；廠商少收的錢會按「還在庫的比例」分攤到存貨成本與銷貨成本。"}
      </p>
      ...
      <p style={{ marginTop: 10, fontSize: 13, color: "var(--red)" }}>
        送出後會立刻入帳並異動庫存，<strong>目前沒有刪除或更正退回單的功能</strong>。
        請先確認數量與金額，尤其是預設帶入的「全退」數字。
      </p>
      <div style={{ marginTop: 10 }}>
        {confirming ? (
          <>
            <span style={{ marginRight: 8, color: "var(--red)" }}>確定要入帳嗎？</span>
            <button className="primary" disabled={busy} onClick={() => void submit()}>
              {busy ? "處理中…" : "確定入帳"}
            </button>{" "}
            <button onClick={() => setConfirming(false)}>再檢查一下</button>
          </>
        ) : (
          <>
...
export function CertCell(props: {
  ...
  if (editing) {
    return (
      <span style={{ whiteSpace: "nowrap" }}>
        ...
        />{" "}
        <input type="date" title="證明單日期（決定申報減項的歸期；未填以退回日計）" value={date} onChange={(e) => setDate(e.target.value)} />{" "}
  ...
  return (
    <span style={props.certNo ? undefined : { color: "var(--red)" }}>
      {props.certNo ? `${props.certNo}${props.certDate ? `（${props.certDate}）` : ""}` : "尚未登錄"}{" "}
