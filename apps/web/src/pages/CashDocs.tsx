...
export function CashDocs() {
  ...
  const voidDoc = async (d: CashDocRow) => {
    const label = d.kind === "receipt" ? "收款單" : "付款單";
    const reason = window.prompt(
      `作廢${label} #${d.id}（${fmt(d.amount)} 元）：請輸入作廢理由。\n` +
        "系統會開一張反向傳票沖平原傳票，這張單沖過的應收／應付會自動回復；金額打錯請作廢後重開一張。",
    );
  ...
  const submit = async () => {
    try {
      if (!partnerId) throw new Error(kind === "receipt" ? "請選客戶" : "請選供應商");
      if (!accountId) throw new Error("請選收付科目（現金/銀行）");
      if (amount <= 0) throw new Error("金額須為正整數");
    ...
    <div>
      ...
      <div className="card">
        ...
        <form className="inline" onSubmit={(e) => e.preventDefault()}>
          ...
          <label className="field">金額（含稅）<input type="number" min={0} value={amount} onChange={(e) => setAmount(Number(e.target.value))} /></label>
          <label className="field">
            收付科目
          ...
          <button className="primary" onClick={submit}>建立</button>
        </form>
        {excess > 0 && (
          <p style={{ fontSize: 13, color: "var(--amber)", margin: "8px 0 0" }}>
            {kind === "receipt"
              ? `本次溢收 ${fmt(excess)} 元將掛「預收款項」（負債）——應收不會變成負數，之後的銷貨可到下方「沖用預收／預付」面板用這筆餘額沖銷`
              : `本次溢付 ${fmt(excess)} 元將掛「預付貨款」（資產）——應付不會變成負數，之後的進貨可到下方「沖用預收／預付」面板用這筆餘額沖銷`}
          </p>
        )}
        {partnerId > 0 && settleDocs.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <h4 style={{ margin: "4px 0" }}>立沖：指定沖哪幾張單（不填則整筆列對象層級，帳齡自動沖最舊）</h4>
            <table>
              <thead>
                <tr>
                  <th>{kind === "receipt" ? "銷貨單" : "進貨單"}（含期初）</th><th>日期</th>
                  {/* 這一欄是退回後的淨額，不是原單金額——標題不講清楚就會與銷貨／進貨頁的數字對不上 */}
                  <th className="num" title="原單金額扣掉已沖回應收／應付的退回、折讓後的淨額">單據金額（退回後）</th>
                  <th className="num">其中已退</th>
                  <th className="num">已沖</th>
                  {/* R6：未指定沖銷的舊收付款依日期自動沖最舊（與帳齡同一條規則）——
                      不分欄顯示的話，「已沖」對不上立沖紀錄，使用者無法歸因 */}
                  <th className="num" title="先前未指定沖銷的收付款餘額，依日期自動沖最舊的單（與帳齡同一條規則）">未指定已抵</th>
                  <th className="num">未沖餘額</th><th className="num">本次沖銷</th>
                </tr>
              </thead>
              <tbody>
                {settleDocs.map((d) => (
                  <tr key={`${d.docType}:${d.id}`}>
                    <td>{d.docType === "opening" ? `期初 #${d.id}` : `#${d.id}`}</td>
                    <td>{d.docDate}</td>
                    <td className="num">{fmt(d.total)}</td>
                    <td className="num" title={d.returned ? `原單金額 ${fmt(d.total + d.returned)} 元，已退 ${fmt(d.returned)} 元` : undefined}>
            ...
            {allocSum > 0 && (
              <p style={{ fontSize: 13, color: allocSum > amount ? "var(--red)" : "var(--green)" }}>
                沖銷合計 {fmt(allocSum)}／收付金額 {fmt(amount)}{allocSum > amount && "——超過收付金額，送出會被擋"}
              </p>
            )}
          </div>
        )}
      ...
      <div className="card">
        <h3>未收／未付餘額（銷貨/進貨＋期初應收付 − 已收付；已沖銷銷貨不計）</h3>
        {balances.data?.length === 0 && (
          <EmptyState
            icon="⚖️"
            title="目前沒有未結清的款項"
            desc="開出銷貨單就會產生應收、開進貨單就會產生應付，這裡會列出每個對象還差多少。全部收付完畢時也會是空的——那是好事。"
          />
        )}
        ...
        <table>
          <thead>
            <tr>
              <th>交易對象</th>
              <th className="num">應收餘額</th>
              {/* 預收/預付與應收/應付分列不互抵（0027，B9）：「他欠我 X」和「我欠他 Y」是兩個事實，
                  淨額會把資產負債表壓成一個假數字 */}
              <th className="num" title="收款超過應收的部分（掛預收款項，屬負債），可沖之後的銷貨">預收餘額</th>
              <th className="num">應付餘額</th>
              <th className="num" title="付款超過應付的部分（掛預付貨款，屬資產），可沖之後的進貨">預付餘額</th>
            </tr>
          </thead>
          <tbody>
            {balances.data?.map((b) => (
      ...
      <div className="card">
        ...
        {docs.data?.length === 0 && (
          <EmptyState
            ...
            title="還沒有收付款單"
            {...(balances.data && balances.data.length > 0
              ? { desc: "收到客戶的錢、或付錢給供應商時，用上面的表單開一張——系統自動拋轉傳票沖銷應收／應付。" }
              : {
                  desc: "要先有銷貨或進貨才會產生應收應付。開一張銷貨單之後回來這裡收款。",
                  actionLabel: "去開銷貨單",
        ...
        <table>
          <thead><tr><th>單號</th><th>類別</th><th>日期</th><th>對象</th><th className="num">金額</th><th className="num" title="建單時超過該對象未沖餘額、掛預收款項／預付貨款的部分">其中預收/預付</th><th>摘要</th><th>狀態</th><th></th></tr></thead>
          <tbody>
            {docs.data?.map((d) => (
              <Fragment key={d.id}>
                <tr>
                  <td>#{d.id}</td>
                  <td>{d.kind === "receipt" ? "收款" : "付款"}</td>
                  <td>{d.docDate}</td>
                  ...
                  <td>
                    {d.voidedAt ? (
                      <span className="badge canceled" title={`作廢理由：${d.voidReason ?? ""}（沖轉傳票 #${d.reversalEntryId ?? "?"}）`}>
                        已作廢
                      </span>
                    ) : (
                      "—"
                  ...
                  <td>
                    <button className="small" onClick={() => void toggleDetail(d.id)}>
                      {expandedId === d.id ? "收合" : "沖銷明細"}
                    </button>{" "}
                    {!d.voidedAt && <button className="small" onClick={() => void voidDoc(d)}>作廢</button>}
                  </td>
                </tr>
...
function DetailPanel({ detail }: { detail: CashDocDetail }) {
  const targetLabel = (a: CashDocDetail["allocations"][number]) =>
    a.targetType === "opening" ? `期初單 #${a.targetId}` : a.targetType === "sale" ? `銷貨單 #${a.targetId}` : `進貨單 #${a.targetId}`;
  return (
    <div style={{ fontSize: 13, padding: "4px 0" }}>
      {detail.allocations.length === 0 ? (
        <p style={{ margin: "4px 0" }}>
          這張單沒有指定沖銷任何單據——{detail.unappliedAmount > 0 ? "扣掉預收/預付的部分" : "整筆"}
          屬對象層級，帳齡與未沖清單會依日期自動沖該對象最舊的單。
        </p>
      ) : (
        <table>
          <thead>
            <tr>
              ...
              <th className="num" title="該單「當下」的未沖餘額（含未指定沖銷收付款的自動分攤）；— 表示原單已作廢或整單沖銷">該單目前剩</th>
              <th>傳票</th>
            </tr>
          </thead>
          <tbody>
            {detail.allocations.map((a, i) => (
              <tr key={i}>
                ...
                <td>{a.fromPrepaid ? `預收/預付沖用（${a.allocDate ?? "?"}）` : "建單立沖"}</td>
      ...
      {detail.unappliedAmount > 0 && (
        <p style={{ margin: "6px 0 0", color: "var(--amber)" }}>
          建單時溢{detail.kind === "receipt" ? "收掛「預收款項」" : "付掛「預付貨款」"} {fmt(detail.unappliedAmount)} 元
          {detail.voidedAt
            ? "（本單已作廢，餘額已由反向傳票收回）"
            : `，已沖用 ${fmt(detail.unappliedAmount - detail.prepaidRemaining)} 元、還剩 ${fmt(detail.prepaidRemaining)} 元可在「沖用預收／預付」面板使用`}
        </p>
      )}
      {detail.voidedAt && detail.allocations.length > 0 && (
        <p style={{ margin: "6px 0 0", color: "var(--text-2)" }}>
          本單已作廢：上列沖銷紀錄僅為軌跡，被沖過的單據未沖餘額已自動回復。
        </p>
      )}
    </div>
  );
...
function ApplyPrepaidCard({ partners, onApplied }: { partners: Partner[]; onApplied: () => void }) {
  ...
  const label = kind === "receipt" ? "預收" : "預付";

  const submit = async () => {
    try {
      if (!from) throw new Error(`請選要動用的${label}餘額（收付款單）`);
      const allocations = Object.entries(alloc)
        .map(([key, v]) => {
          const [targetType, targetId] = key.split(":");
          return { targetType, targetId: Number(targetId), amount: v || 0 };
        ...
        .filter((a) => a.amount > 0);
      if (!allocations.length) throw new Error("請在下方填要沖哪幾張單、各沖多少");
      await api.post(`/cash-docs/${from.id}/apply-prepaid`, { applyDate, allocations });
      setError(null);
      setOkMsg(`已用${kind === "receipt" ? "收款單" : "付款單"} #${from.id} 的${label}餘額沖銷 ${fmt(sum)} 元（自動拋轉傳票）`);
      setAlloc({});
      await load();
      onApplied();
    } catch (e) {
  ...
  return (
    <div className="card">
      <h3>沖用預收／預付（不再收付現金，直接用先前溢收溢付的餘額沖銷單據）</h3>
      {error && <div className="error">{error}</div>}
      ...
      <form className="inline" onSubmit={(e) => e.preventDefault()}>
        <label className="field">
          類別
          <select value={kind} onChange={(e) => { setKind(e.target.value as "receipt" | "payment"); setPartnerId(0); }}>
            <option value="receipt">預收（沖客戶的銷貨）</option>
            <option value="payment">預付（沖供應商的進貨）</option>
          </select>
        </label>
        <label className="field">
          {kind === "receipt" ? "客戶" : "供應商"}
        ...
        <label className="field">沖用日<input type="date" value={applyDate} onChange={(e) => setApplyDate(e.target.value)} /></label>
        <label className="field">
          動用哪張單的{label}餘額
          <select value={fromId} onChange={(e) => { setFromId(Number(e.target.value)); setAlloc({}); }}>
            ...
            {prepaid.map((d) => (
              <option key={d.id} value={d.id}>
                #{d.id}（{d.docDate}，剩 {fmt(d.remaining)} 元）
              </option>
            ))}
          </select>
        </label>
        <button className="primary" onClick={() => void submit()}>沖銷</button>
      </form>
      {partnerId > 0 && prepaid.length === 0 && (
        <p style={{ fontSize: 13, color: "var(--text-2)" }}>
          這個對象沒有{label}餘額——只有收付金額超過當時未沖餘額的收付款單才會產生{label}。
        </p>
      )}
      {from && targets.length === 0 && (
        <p style={{ fontSize: 13, color: "var(--text-2)" }}>沒有可沖銷的未沖單據，先開{kind === "receipt" ? "銷貨" : "進貨"}單再回來沖。</p>
      )}
      {from && targets.length > 0 && (
        <table>
          <thead>
      ...
      {from && sum > 0 && (
        <p style={{ fontSize: 13, color: sum > from.remaining ? "var(--red)" : "var(--green)" }}>
          沖銷合計 {fmt(sum)}／{label}餘額 {fmt(from.remaining)}
          {sum > from.remaining && "——超過餘額，送出會被擋"}
        </p>
      )}
    </div>
  );
