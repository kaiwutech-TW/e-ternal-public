...
function SalePrintView(props: { sale: SaleDetail; company: CompanyHeader | null; onClose: () => void }) {
  ...
  return (
    <PrintOverlay onClose={props.onClose}>
      <CompanyHeaderBlock company={props.company} docTitle="出貨單（銷貨單）" />
      <div className="meta-row">
        <div>
          客戶：{p?.name ?? sale.partnerName}
          {p?.taxId ? `（統編 ${p.taxId}）` : ""}
          {p?.contactPerson ? `　聯絡人 ${p.contactPerson}` : ""}
          {p?.phone ? `　電話 ${p.phone}` : ""}
        </div>
        <div>單號：#{sale.id}</div>
      </div>
      <div className="meta-row">
        <div>送貨地址：{p?.shipToAddress ?? p?.address ?? "＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿"}</div>
        <div>出貨日期：{sale.docDate}{sale.dueDate ? `　收款到期：${sale.dueDate}` : ""}</div>
      </div>
      <table>
        <thead>
          <tr><th>品名</th><th>編號</th><th className="num">數量</th><th>單位</th><th className="num">單價（未稅）</th><th className="num">金額</th></tr>
        </thead>
        <tbody>
          {sale.lines.map((l) => (
            <tr key={l.id}>
      ...
      <div className="totals">
        <div>
          未稅合計：{fmt(sale.subtotal)} 元　營業稅：{fmt(sale.tax)} 元
          {sale.taxType === "2" && `（零稅率・${sale.zeroTaxViaCustoms ? "經海關出口" : "非經海關"}${sale.zeroTaxCertNo ? `・證明文件 ${sale.zeroTaxCertNo}` : ""}）`}
        </div>
        <div className="grand">總計：{fmt(sale.total)} 元</div>
      </div>
      <div className="sign-row">
        <div className="sign-box">出貨人簽章</div>
        <div className="sign-box">送貨人簽章</div>
        <div className="sign-box">客戶簽收（請核對品項數量）</div>
      </div>
      <div className="foot-note">本單為出貨與簽收憑據；發票另行開立。金額為整數新台幣元。</div>
    </PrintOverlay>
  );
}
...
export function Sales() {
  ...
  const confirmIssueB2C = (saleId: number) => {
    if (b2cKind === "donate") {
      if (!b2cNpoban.trim()) {
        setError("捐贈發票要填受贈機構的捐贈碼（愛心碼）——向對方或財政部愛心碼查詢平台取得");
        return;
      }
    ...
    if (b2cKind !== "none") {
      ...
      if (!type || !id) {
        setError("載具要填類別與號碼（手機條碼載具請輸入 / 開頭的條碼）");
        return;
      }
  ...
  const backfillZeroCert = async (d: DocRow) => {
    const label = d.zeroTaxViaCustoms ? "出口報單號碼" : "外匯證明等文件號碼";
    const certNo = window.prompt(
      `銷貨單 #${d.id} 是零稅率（${d.zeroTaxViaCustoms ? "經海關出口" : "非經海關"}）。\n` +
        `請輸入${label}（系統只登錄號碼，不驗證文件內容）：`,
      d.zeroTaxCertNo ?? "",
    );
  ...
  const voidReturn = async (r: ReturnRow) => {
    const label = r.kind === "return" ? "退回單" : "折讓單";
    const reason = window.prompt(
      `作廢${label} #${r.id}（${fmt(r.total)} 元）：請輸入作廢理由。\n` +
        (r.kind === "return"
          ? "回補過的庫存會再沖出去（退回來的貨已再賣出時會被擋下）、傳票以反向分錄沖平。"
          : "傳票以反向分錄沖平；已登錄證明單的折讓單，作廢後請下載 G0501 作廢訊息上傳。"),
    );
  ...
  const voidSale = async (d: DocRow) => {
    const reason = window.prompt(
      `作廢銷貨單 #${d.id}（${fmt(d.total)} 元）：請輸入作廢理由。\n` +
        "庫存會按原出庫成本回補、傳票以反向分錄沖平；單子打錯請作廢後重開一張正確的。",
    );
    if (reason === null) return;
    try {
    ...
    <div>
      ...
      <details className="card" style={{ paddingTop: 10 }}>
        <summary style={{ cursor: "pointer", fontWeight: 600 }}>
          該用「退貨／折讓」還是「作廢發票」？（點開看四個詞的差別）
        </summary>
        <ul style={{ fontSize: 13, lineHeight: 1.9, margin: "8px 0 0", paddingLeft: 20 }}>
          <li>
            <strong>退貨</strong>：貨真的回到倉庫。庫存回補、成本按原銷貨成本回沖，
            以<strong>退貨當期</strong>入帳，原銷貨單金額不變。← 多數情況用這個
          </li>
          <li>
            <strong>折讓</strong>：貨沒退，但要少收錢（破損議價、量大回饋、尾數抹零）。不動庫存。
          </li>
          <li>
            <strong>作廢發票</strong>：<strong>發票本身開錯了</strong>（金額打錯、統編錯）且發票還拿得回來。
            在「電子發票」頁操作。發票日期所屬月份已關帳時會被擋下——那一期可能已經申報過，
            此時唯一的路是開退貨／折讓單。
          </li>
          <li>
            <strong>沖銷</strong>：畫面上有兩個意思，看位置——
            這一頁的「已沖銷」標記指<strong>整張銷貨單被迴轉</strong>（只會由作廢發票時連帶產生）；
            「收付款」頁的沖銷指<strong>收到的錢沖掉哪幾張單的應收</strong>，兩者無關。
          </li>
          <li style={{ color: "var(--red)" }}>
            <strong>單子打錯</strong>：還沒開發票（或發票已作廢）的銷貨單可以直接按「作廢」——
            庫存回補、傳票沖平、原單留痕，然後重開一張正確的。已開發票且未作廢的要先作廢發票；
            該月已關帳的只能走退貨／折讓（以當期認列）。
          </li>
        </ul>
      </details>
      <div className="card">
        ...
        {zeroMissingCert.length > 0 && (
          <p style={{ margin: "0 0 8px", fontSize: 13, color: "var(--red)" }}>
            有 <strong>{zeroMissingCert.length} 筆零稅率銷貨</strong>還沒登錄證明文件號碼
            （經海關＝出口報單號碼；非經海關＝外匯證明文件號碼等）。
            申報零稅率銷售額需檢附證明文件——取得號碼後按該列的「補登證明文件」填入；
            系統只登錄號碼，不驗證文件真偽。
          </p>
        )}
        <table>
          <thead>
            <tr><th>單號</th><th>日期</th><th>收款到期</th><th className="num">未稅</th><th className="num">稅額</th><th className="num">總額</th><th>發票</th><th>退貨</th><th></th></tr>
          </thead>
          <tbody>
            {docs.data?.map((d) => {
                ...
                <tr>
                    ...
                    {d.taxType === "2" && (
                      <>
                        ...
                        <span
                          className="badge"
                          title={
                            (d.zeroTaxViaCustoms ? "經海關出口" : "非經海關") +
                            (d.zeroTaxCertNo ? `｜證明文件 ${d.zeroTaxCertNo}` : "｜證明文件號碼未登錄")
                          }
                          style={d.zeroTaxCertNo ? undefined : { background: "var(--red-tint)", color: "var(--red)" }}
                        >
                          零稅率{d.zeroTaxCertNo ? "" : "・缺證明"}
                        </span>
                      </>
                    )}
                  </td>
                  ...
                  <td>
                    {inv ? (
                      <>
                        {inv.invoiceNumber}{" "}
                        <span className={`badge ${inv.status}`}>{inv.status === "issued" ? "已開立" : "已作廢"}</span>
                      </>
                    ) : (
                      "未開立"
                    )}{" "}
                    {d.reversalEntryId && (
                      <span className="badge canceled" title={d.voidReason ? `作廢理由：${d.voidReason}` : "整張銷貨單已迴轉"}>
                        已沖銷
                      </span>
                    )}
                  </td>
                  ...
                  <td>
                    {(!inv || inv.status === "canceled") && !d.reversalEntryId && (
                      <>
                        <button className="small" onClick={() => void issue(d.id, "B2B")}>{inv ? "重開 B2B" : "開 B2B"}</button>{" "}
                        <button className="small" onClick={() => startIssueB2C(d.id)}>{inv ? "重開 B2C" : "開 B2C"}</button>{" "}
                      </>
                    )}
                    {!d.reversalEntryId && returningId !== d.id && (
                      <button className="small" onClick={() => { setReturningId(d.id); setError(null); }}>退貨／折讓</button>
                    )}{" "}
                    {/* 作廢（B4）：有 issued 發票或已沖銷的單不顯示——伺服端同樣會擋，這裡只是不誘導 */}
                    {canVoid && (!inv || inv.status === "canceled") && !d.reversalEntryId && (
                      <>
                        <button className="small" onClick={() => void voidSale(d)}>作廢</button>{" "}
                      </>
                    )}
                    {d.taxType === "2" && !d.reversalEntryId && (
                      <>
                        <button className="small" onClick={() => void backfillZeroCert(d)}>
                          {d.zeroTaxCertNo ? "改證明文件" : "補登證明文件"}
                        </button>{" "}
                      </>
                    )}
                    <button className="small" onClick={() => void openPrint(d.id)}>列印</button>
                  </td>
                </tr>
                {issuingB2CId === d.id && (
                  <tr>
                    <td colSpan={9}>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", fontSize: 13 }}>
                        <strong>開立 B2C 發票</strong>
                        <label className="field">
                          載具／捐贈
                          <select value={b2cKind} onChange={(e) => setB2cKind(e.target.value as typeof b2cKind)}>
                            <option value="none">無（印紙本證明聯）</option>
                            <option value="3J0002">手機條碼載具</option>
                            <option value="CQ0001">自然人憑證載具</option>
                            <option value="custom">其他載具（自填類別代號）</option>
                            <option value="donate">捐贈（愛心碼）</option>
                          </select>
                        </label>
                        {b2cKind === "custom" && (
                          <input
                        ...
                        {(b2cKind === "3J0002" || b2cKind === "CQ0001" || b2cKind === "custom") && (
                          <input
                            placeholder={b2cKind === "3J0002" ? "/ 開頭的手機條碼" : "載具號碼"}
                            style={{ width: 140 }}
                            value={b2cCarrierId}
                        ...
                        <button className="small" onClick={() => confirmIssueB2C(d.id)}>開立</button>
                        <button className="small" onClick={() => setIssuingB2CId(null)}>取消</button>
                        <span style={{ color: "var(--text-2)" }}>
                          存載具或捐贈的發票不印證明聯；「無」維持現行預設。載具號碼與捐贈碼系統不驗證真偽，請照消費者出示的條碼輸入。
                        </span>
                      </div>
                    </td>
                  </tr>
      ...
      <div className="card">
        <h3>退貨／折讓紀錄（原銷貨單維持原金額，這裡是它的減項）</h3>
        {missingCert.length > 0 && (
          ...
          <p style={{ margin: "0 0 8px", fontSize: 13, color: "var(--red)" }}>
            ...
            <strong>折讓單</strong>的號碼由貴公司自行編定，補登號碼＋日期後可直接在本頁下載
            G0401 折讓證明單 XML（放進 Turnkey 上傳目錄即可）；<strong>退回單</strong>的證明單
            仍需以財政部軟體或加值中心平台開立後回來補登。沒補登的，申報時就少一筆減項的依據。{" "}
        ...
        {returns.data?.length === 0 && (
          <EmptyState
            ...
            title="還沒有退貨或折讓"
            desc={
              "客戶把貨退回來，就在上面的銷貨單列按「退貨／折讓」——庫存會自動回補、成本按原銷貨成本回沖。" +
              "貨沒退但要少收錢（破損議價、量大回饋）請選「折讓」，那不會動到庫存。"
            }
          />
        )}
        {returns.data && returns.data.length > 0 && (
          <table>
            <thead>
              <tr>
                <th>單號</th><th>日期</th><th>客戶</th><th>類型</th><th>原銷貨單</th>
                <th className="num">未稅</th><th className="num">稅額</th><th className="num">沖應收</th>
                <th className="num">應退客戶</th>
                {/* 證明單號要看得到：不然使用者填了號碼卻在任何列表都找不到，開單時填的等於丟進黑洞 */}
                <th title="折讓：號碼自編，補登號碼＋日期後可下載 G0401 XML（Turnkey 上傳）。退回：仍以外部（財政部軟體或加值中心平台）開立後補登號碼">證明單</th>
                <th>原因</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              {(certOnly ? missingCert : returns.data).map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.docDate}</td>
                  <td>{r.partnerName}</td>
                  <td><span className="badge">{r.kind === "return" ? "退回" : "折讓"}</span></td>
                  <td>#{r.saleId}</td>
                  ...
                  <td>
                    ...
                    {r.kind === "allowance" && r.certNo && (
                      <>
                        ...
                        <button
                          className="small"
                          title="下載 G0401 折讓證明單 XML（檔名照 MIG 慣例，放進 Turnkey 上傳目錄）"
                    ...
                    {r.kind === "allowance" && r.certNo && r.voidedAt && (
                      <>
                        ...
                        <button
                          className="small"
                          title="下載 G0501 作廢折讓證明單 XML（原 G0401 已上傳過的，要一併上傳這份作廢訊息）"
      ...
      <div className="card">
        <h3>應收帳齡（催款用：收款自動沖最舊的銷貨單，剩下的按**收款到期日**分逾期桶）</h3>
        ...
        <table style={{ marginTop: 12 }}>
          <thead>
            <tr>
              ...
              <th className="num">未收合計</th>
              <th className="num">已到期未收</th>
              <th className="num">預收</th>
            </tr>
          </thead>
          <tbody>
        ...
        {aging.data?.rows.length === 0 && <p style={{ fontSize: 13, color: "var(--text-2)" }}>目前沒有未收款項。</p>}
      </div>
      {printSale && (
